...
class Tracker(object):
    ...
    def _get_background(self, numframes=120):
    ...
    def process_video(self):
        ...
        while(self.video.grab() and self.video.isOpened()):
            ret, inFrame = self.video.retrieve()
            frameNum += 1
            all_targets.append(self._process_frame(inFrame, frameNum))

        return all_targets

    def _process_frame(self, inFrame, frameNum):
        grayframe = cv2.cvtColor(inFrame, cv2.COLOR_BGR2GRAY)
        ...
        return self._detect_targets(thresh_img, inFrame, frameNum)

    def _detect_targets(self, thresh_img, inFrame, frameNum):
